-- Complete Analytics Functions Setup for Supabase
-- This script will create all necessary analytics functions
...
DROP FUNCTION IF EXISTS get_average_order_value(TEXT, TEXT);
DROP FUNCTION IF EXISTS get_sales_comparison(DATE, DATE, DATE, DATE);
DROP FUNCTION IF EXISTS get_sales_comparison(TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_hourly_sales_trend(DATE);
DROP FUNCTION IF EXISTS get_hourly_sales_trend(TEXT);
...
-- 1. Sales Trend Function (single function with TEXT parameters)
...
BEGIN
  ...
  ELSE
    -- Default to daily grouping - FIXED: Include to_char(sale_date, 'Mon DD') in GROUP BY
    RETURN QUERY
    SELECT 
    ...
    FROM sales
    WHERE DATE(sale_date) >= p_start_date::DATE AND DATE(sale_date) <= p_end_date::DATE
    GROUP BY DATE(sale_date), to_char(sale_date, 'Mon DD')
    ORDER BY period_date;
  END IF;
END;
$$ LANGUAGE plpgsql;

-- 2. Average Order Value Function
...
-- 4. Hourly Sales Trend Function - FIXED: Include to_char(sale_date, 'HH24:00') in GROUP BY
...
-- 5. Sales Summary Metrics Function
...
-- 6. Peak Hours Analysis Function
...
BEGIN
  ...
  -- Test get_sales_trend
  IF EXISTS (SELECT 1 FROM get_sales_trend('2025-06-01', '2025-06-30', 'day') LIMIT 1) THEN
    RAISE NOTICE 'get_sales_trend function works correctly';
  ELSE
    RAISE NOTICE 'get_sales_trend function returned no data (this is normal if no sales exist)';
  END IF;
  
  -- Test get_average_order_value
  IF EXISTS (SELECT 1 FROM get_average_order_value('2025-06-01', '2025-06-30') LIMIT 1) THEN
    RAISE NOTICE 'get_average_order_value function works correctly';
  ELSE
    RAISE NOTICE 'get_average_order_value function returned no data (this is normal if no sales exist)';
  END IF;
  
  -- Test get_sales_summary_metrics
  IF EXISTS (SELECT 1 FROM get_sales_summary_metrics('2025-06-01', '2025-06-30') LIMIT 1) THEN
    RAISE NOTICE 'get_sales_summary_metrics function works correctly';
  ELSE
    RAISE NOTICE 'get_sales_summary_metrics function returned no data (this is normal if no sales exist)';
  END IF;
  
  RAISE NOTICE 'All analytics functions have been created successfully!';
END $$; 