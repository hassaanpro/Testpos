-- Fix BNPL Payment Function - Reference ID Type Issue
-- This fixes the UUID type mismatch in the cash_ledger table

-- Drop the existing function if it exists
DROP FUNCTION IF EXISTS process_bnpl_payment(TEXT, NUMERIC);

-- Create the fixed process_bnpl_payment function
...
BEGIN
  ...
  -- Record cash ledger entry for BNPL payment (FIXED: Use UUID for reference_id)
  INSERT INTO cash_ledger (
    ...
    'bnpl_transaction',
    v_bnpl_id, -- Use the UUID directly instead of p_bnpl_id (TEXT)
    v_current_time,
    v_current_time
  );
